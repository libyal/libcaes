...
TEST_FUNCTIONS="context support tweaked_context";
...
test_python_function()
{
	...
	local TEST_SCRIPT="${TEST_TOOL_DIRECTORY}/pycaes_test_${TEST_FUNCTION}.py";
...
test_python_function_with_input()
{
	...
	run_test_on_input_directory "pycaes" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_SCRIPT}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
