...
typedef struct caes_test_vector_256bit caes_test_vector_256bit_t;

struct caes_test_vector_256bit
{
	/* Key data
	 */
	uint8_t key_data[ 32 ];
...
caes_test_vector_256bit_t caes_crypt_ccm_test_vectors_256bit[ 1 ] = {
 ...
 * Returns 1 if successful, 0 if not or -1 on error
 */
int caes_test_crypt_ccm_decryption_256bits(
     void )
{
	...
	for( test_index = 0;
	     ...
	     test_index++ )
	{
		test_vector_256bit = &( caes_crypt_ccm_test_vectors_256bit[ test_index ] );

		result = caes_test_crypt_ccm(
		          LIBCAES_CRYPT_MODE_DECRYPT,
		          test_vector_256bit->key_data,
		          256,
		          test_vector_256bit->nonce,
		          12,
 ...
 * Returns 1 if successful, 0 if not or -1 on error
 */
int caes_test_crypt_ccm_encryption_256bits(
     void )
{
	caes_test_vector_256bit_t *test_vector_256bit = NULL;
	int result                                    = 0;
...
#endif
{
	...
	if( caes_test_crypt_ccm_decryption_256bits() != 1 )
	{
		return( EXIT_FAILURE );
	}
	if( caes_test_crypt_ccm_encryption_256bits() != 1 )
	{
		return( EXIT_FAILURE );
	}
	return( EXIT_SUCCESS );
