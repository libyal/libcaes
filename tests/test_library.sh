...
LIBRARY_TESTS="context error key support tweaked_context";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./caes_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libcaes" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
