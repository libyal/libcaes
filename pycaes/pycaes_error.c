      ...
      format_string )
{
	...
	char error_string[ PYCAES_ERROR_STRING_SIZE ];
	char exception_string[ PYCAES_ERROR_STRING_SIZE ];

	static char *function     = "pycaes_error_raise";
	...
	if( error != NULL )
	{
		if( libcerror_error_backtrace_sprint(
		     error,
		     error_string,
		     PYCAES_ERROR_STRING_SIZE ) != -1 )
		{
			while( error_string_index < PYCAES_ERROR_STRING_SIZE )
			{
			...
			if( error_string_index >= PYCAES_ERROR_STRING_SIZE )
			{
				error_string[ PYCAES_ERROR_STRING_SIZE - 1 ] = 0;
			}
			PyErr_Format(
			 exception_object,
			 "%s %s",
