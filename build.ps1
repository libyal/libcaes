...
Else
{
	...
	If (-Not (Test-Path "${VSSolutionPath}"))
	{
		...
		Invoke-Expression -Command "& '${Python}' ${MSVSCppConvert} --output-format ${VisualStudioVersion} ${VSToolsOptions} msvscpp\libcaes.sln 2>&1" | %{ "$_" }
	}
}
If (${VisualStudioVersion} -eq "2026")
{
	$VSSolutionFile = "${VSSolutionPath}\libcaes.slnx"
}
Else
{
	$VSSolutionFile = "${VSSolutionPath}\libcaes.sln"
}
